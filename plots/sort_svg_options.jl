...
from `results/<GPU_TAG>/sort.csv`. Each variant exports
...
Run:
  julia --project=plots plots/sort_svg_options.jl
  # or override the GPU tag:
  KF_GPU_TAG=A40 julia --project=plots plots/sort_svg_options.jl
=#
...
const OUT_DIR  = joinpath(get(ENV, "KF_FIGURES_ROOT", joinpath(REPO_ROOT, "figures")), GPU_TAG)
mkpath(OUT_DIR)
