...
const BENCH_COLORS = Dict(
    ...
    "CUB" => colorant"#00008B",
    # AMD counterparts. The bench scripts emit "Base" where CUDA emits
    # "CUDA" and "rocPRIM"/"rocBLAS gemm_ex" where CUDA emits "CUB"/
    # "cuBLAS" (see benchmarks/*_perf_comparison.jl). Same role → same
    # colour, so an NVIDIA and an AMD figure read identically.
    "Base" => colorant"#CC79A7",
    "rocPRIM" => colorant"#00008B",
    "rocBLAS gemm_ex" => colorant"#56B4E9",
)
...
# ---------------------------------------------------------------------------
# Backend-aware method names
#
# The CSVs name the vendor baseline after the backend that produced them:
# CUB / cuBLAS / CUDA on NVIDIA, rocPRIM / rocBLAS / Base on AMD. Plot code
# must therefore ASK the DataFrame instead of hardcoding "CUB" — an AMD CSV
# plotted with the NVIDIA names silently drops the vendor bars entirely.
# ---------------------------------------------------------------------------

_methods_in(df) = Set(string.(df.method))

"Vendor primitive-library baseline present in `df` (\"CUB\" or \"rocPRIM\")."
vendor_method(df) = "rocPRIM" in _methods_in(df) ? "rocPRIM" : "CUB"

"""
    vendor_blas_method(df) -> String

Vendor BLAS baseline present in `df`. On AMD `vendor_matvec_baseline`
(perfs/julia/vendor_int8.jl) labels the float path `LinearAlgebra` — it is
`A * x`, i.e. rocBLAS `gemv` — and reserves `rocBLAS gemm_ex` for the 1-byte
types. Pick the float row: the np-plots compare a single dtype.
"""
function vendor_blas_method(df)
    ms = _methods_in(df)
    "cuBLAS" in ms && return "cuBLAS"
    "LinearAlgebra" in ms && return "LinearAlgebra"
    return "rocBLAS gemm_ex"
end

"Display name for a vendor BLAS method (`LinearAlgebra` = rocBLAS `gemv`)."
vendor_blas_label(m::AbstractString) = m == "LinearAlgebra" ? "rocBLAS" :
                                       String(first(split(m)))

"Array-library baseline present in `df` (\"CUDA\" or \"Base\")."
array_method(df) = "Base" in _methods_in(df) ? "Base" : "CUDA"

"""
    reduce_method_order(df) -> Vector{String}

`REDUCE_METHOD_ORDER` resolved against the backend that produced `df`.
"""
reduce_method_order(df) = [array_method(df), "AK", "Forge", vendor_method(df)]

"""
    _add_heading!(fig, supertitle, subtitle)

Prepend a bold title (and an optional smaller grey subtitle) above every
panel. Uses row 0 / -1 so the caller's `fig[1, …]` panel layout and
`fig[2, :]` legend stay valid.
"""
function _add_heading!(fig, supertitle, subtitle)
    supertitle === nothing && return fig
    # tellwidth=false is REQUIRED: a Label that reports its width forces the
    # whole grid as wide as one line of text, which collapses the panels.
    # word_wrap then breaks the line at the figure width instead of
    # overflowing past both edges.
    Label(fig[-1, :], supertitle; fontsize=22, font=:bold,
          halign=:center, tellwidth=false, word_wrap=true,
          padding=(0, 0, 2, 2))
    if subtitle !== nothing
        Label(fig[0, :], subtitle; fontsize=15, color=(:black, 0.6),
              halign=:center, tellwidth=false, word_wrap=true,
              padding=(0, 0, 0, 8))
    end
    return fig
...
function plot_grouped_barplot_multi(
    ...
    # Figure-level heading. Panels only say "n = …", so without this a
    # reader who meets the PNG outside its README has no idea which op,
    # which GPU or which direction is better.
    supertitle::Union{Nothing,AbstractString}=nothing,
    ...
    # One narrow panel can't fit the whole legend on a single line.
    Legend(fig[2, :], elems, lbls; orientation=:horizontal,
        tellheight=true, tellwidth=false,
        nbanks=length(sizes) == 1 ? 2 : 1)
    _add_heading!(fig, supertitle, subtitle)
...
function plot_npbar(
    ...
    time_unit::String="μs",
    vendor_method::String=vendor_blas_method(df),
    vendor_label::String=vendor_blas_label(vendor_method),
)
    ...
    function get_series(d, method)
        ...
        for v in x_vals
            ...
            if !isempty(row) && isfinite(row.mean_kernel_μs[1])
                ki = row.mean_kernel_μs[1] / time_scale
                ti = isfinite(row.mean_total_μs[1]) ? row.mean_total_μs[1] / time_scale : ki
                push!(k, ki)
    ...
    ck, co, ce = get_series(subset, vendor_method)
    ...
    barplot!(ax, x .+ off, ck, width=w, color=colors[2], label="$vendor_label (kernel)")
    barplot!(ax, x .+ off, co, width=w, color=(cc, overhead_alpha), offset=ck, label="$vendor_label (overhead)")
...
function plot_npbar_multi(
    ...
    supertitle::Union{Nothing,AbstractString}=nothing,
    subtitle::Union{Nothing,AbstractString}=nothing,
    # When set, vendor bars taller than `clip_factor × tallest Forge bar`
    # in that panel are clipped to that ceiling, hatched, and labelled
    # with their REAL value. Without it a single degenerate vendor cell
    # (rocBLAS gemm_ex on n=1 is 220 ms vs Forge's 339 µs) flattens every
    # other bar in the panel to one pixel.
    clip_factor::Union{Real,Nothing}=nothing,
)
    fig = Figure(size=figsize)
    fc = Makie.to_color(colors[1])
    ...
    for (col, total) in enumerate(totals)
        ...
        function get_series(d, method, scale)
            ...
            for v in x_vals
                row = filter(r -> getproperty(r, x_col) == v && r.method == method, d)
                # A vendor cell that errored on this shape is recorded as
                # NaN by the bench script — treat it as "no bar" (and no
                # label), never let it reach the formatter (Int(NaN) throws).
                if !isempty(row) && isfinite(row.mean_kernel_μs[1])
                    ki = row.mean_kernel_μs[1] / scale
                    ti = isfinite(row.mean_total_μs[1]) ? row.mean_total_μs[1] / scale : ki
                    push!(k, ki)
                    push!(ov, max(0.0, ti - ki))
        ...
        fk, fo, fe, f_got = get_series(subset, "Forge", t_scale)
        ck, co, ce, c_got = get_series(subset, vendor_method, t_scale)
        ...
        forge_max = maximum(fk[i] + max(fo[i], isempty(fo_alt) ? 0.0 : fo_alt[i]) + fe[i]
                            for i in eachindex(x_vals); init=0.0)
        full_max = max(forge_max, maximum(ck .+ co .+ ce; init=0.0))
        # Ceiling the vendor bars are clipped to (see `clip_factor`).
        bar_cap = clip_factor === nothing ? full_max :
                  min(full_max, max(forge_max * clip_factor, eps()))
        clipped = ck .+ co .> bar_cap
        ck_drawn = min.(ck, bar_cap)
        co_drawn = max.(0.0, min.(ck .+ co, bar_cap) .- ck_drawn)
        ce_drawn = ifelse.(clipped, 0.0, ce)
        max_height = bar_cap
        fixed_offset = max_height * label_offset_frac

        ax = Axis(fig[1, col];
            xlabel=xlabel, ylabel="Time ($t_unit)",
            title="n×p = $(format_number(total))$title_suffix",
            xticks=(1:length(x_vals), string.(x_vals)), xticklabelrotation=π / 6,
            xlabelsize=16, ylabelsize=16, titlesize=18,
        )
        any(clipped) && ylims!(ax, 0, bar_cap * (1 + 6 * label_offset_frac))
        x = collect(1:length(x_vals))
        w = 0.35
        off = w / 2
        ...
        barplot!(ax, x .+ off, ck_drawn, width=w, color=colors[2])
        barplot!(ax, x .+ off, co_drawn, width=w, color=(cc, overhead_alpha), offset=ck_drawn)

        # Hatched band on clipped bars: this bar runs off the top, its
        # label is the real value.
        for (i, xi) in enumerate(x)
            clipped[i] || continue
            y0, y1 = bar_cap * 0.965, bar_cap * 0.995
            poly!(ax, Point2f[(xi + off - w / 2, y0), (xi + off + w / 2, y0),
                    (xi + off + w / 2, y1), (xi + off - w / 2, y1)];
                color=(:white, 0.0), strokecolor=:black, strokewidth=0.8)
            for frac in (0.25, 0.5, 0.75)
                xh = xi + off - w / 2 + w * frac
                lines!(ax, [xh, xh], [y0, y1]; color=:black, linewidth=0.6)
            end
        end
        ...
        errorbars!(ax, x .- off, fk, fe, color=:black, whiskerwidth=6)
        errorbars!(ax, x .+ off, ck_drawn, ce_drawn, color=:black, whiskerwidth=6)
        ...
        for (i, xi) in enumerate(x)
            ...
            # Clipped bars keep the REAL value as their label.
            if c_got[i]
                text!(ax, xi + off, ck_drawn[i] + co_drawn[i] + fixed_offset;
                    text=label_fmt_fn(ck[i]), align=(:center, :bottom), fontsize=12)
            end
        end
    end
    ...
    legend_lbls = if df2 !== nothing
        ["Forge (kernel)", "Forge (overhead)", "Forge (overhead, Opt)",
            "$vendor_label (kernel)", "$vendor_label (overhead)"]
    else
        ["Forge (kernel)", "Forge (overhead)", "$vendor_label (kernel)", "$vendor_label (overhead)"]
    end
